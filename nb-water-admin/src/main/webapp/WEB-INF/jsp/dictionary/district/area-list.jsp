...
<script>
	...
	layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[ //标题栏
                {type:'checkbox'}
                ,{field: 'areaid', title: '地区代码', minWidth: 80,align : 'center',unresize:true}
                ,{field: 'area', title: '名称', minWidth: 300,align : 'center',unresize:true}
                ,{title: '操作', width: 200,toolbar:'#handle',align : 'center',unresize:true}
            ]]
        	,url: '${ctx}/area/listData.action?father=${father}'
            ,id:'myTable'
            ,even: true
            ,page: true //是否显示分页
            ,limits: [5,10,20]
        ...
        active = {
			addProvince: function(){
				layer.open({
	        		  ...
	        		  fixed: false, //不固定
	        		  maxmin: true,
	        		  content: '${ctx}/area/input.action?father=${father}'
	        		});
			},
        ...
        table.on('tool(myTable)', function(obj){
          ...
          if(obj.event === 'detail'){
        	  layer.open({
        		  ...
        		  fixed: false, //不固定
        		  maxmin: true,
        		  content: '${ctx}/area/info.action?id='+data.id
        		});
          } else if(obj.event === 'del'){
        	  layer.confirm("请问是否确定删除，删除后不可恢复!", {
                  btn: ["确定","取消"] //按钮
              }, function(index){
            	  $.post("${ctx}/area/delete.action?id="+data.id);
                  layer.close(index);
              	 location.reload();
              });
        	  ...
        	  layer.open({
        		  ...
        		  fixed: false, //不固定
        		  maxmin: true,
        		  content: '${ctx}/area/input.action?id='+data.id+"&father=${father}"
        		});
          }
        });
