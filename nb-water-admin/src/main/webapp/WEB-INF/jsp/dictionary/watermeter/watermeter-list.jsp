						...
						<div class="layui-card-body">
							<div class="demoTable marginBottom" id="buttons">
								<div class="layui-btn-group">
									<button class="layui-btn" data-method="addWatermeter">
										<i class="layui-icon">&#xe654;</i> 新增
									</button>
									<button class="layui-btn" data-method="deleteWatermeter">
...
<script type="text/javascript">
    ...
    function tableData() {
        ...
        avg = (clientWidth-161) / 5;//分割5等分，表示表格的5列
            ...
            table.render({
                ...
                cols : [ [ //标题栏
                    ...
                    , {
                        ...
                        minWidth:120,
                        width:avg+2,
                        toolbar : '#handle',align : 'center',unresize:true
                    }
                ] ],
                url : '${ctx}/watermeter/listData.action',
                id : 'myTable',
                page : true, //是否显示分页
                limits : [ 15, 20, 50 ],
                limit : 15 ,//每页默认显示的数量
            ...
            active = {
                addWatermeter : function() {
                    var index = layer.open({
                        ...
                        fixed : false, //不固定
                        maxmin : true,
                        content : '${ctx}/watermeter/input.action'
                    });
                },
                deleteWatermeter : function() {
                    ...
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/watermeter/delete.action", //路径
                            data : {
                                "id" : ids
                            }, //数据，这里使用的是Json格式进行传输
                            dataType : "json",
            ...
            table.on('tool(myTable)', function(obj) {
                ...
                if (obj.event === 'detail') {
                    layer.open({
                        ...
                        fixed : false, //不固定
                        maxmin : true,
                        content : '${ctx}/watermeter/info.action?id=' + data.id
                    });
                } else if (obj.event === 'del') {
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        btn : [ "确定", "取消" ] //按钮
                    }, function(index) {
                        $.post("${ctx}/watermeter/delete.action?id=" + data.id);
                        layer.close(index);
                        raloadt();
                    });
                    ...
                    layer.open({
                        ...
                        fixed : false, //不固定
                        maxmin : true,
                        content : '${ctx}/watermeter/input.action?id=' + data.id
                    });
                }
            });
