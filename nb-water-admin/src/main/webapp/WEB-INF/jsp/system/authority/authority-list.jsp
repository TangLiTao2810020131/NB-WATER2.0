    ...
    function tableData() {
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,url: '${ctx}/authority/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [10,20,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                ...
                inResource:function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authority/inResource.action?id='+data[0].id
                    });
                },
                deleteUsers:function(){
                    ...
                    layer.confirm("删除该权限后,相关用户不可操作，是否确认删除!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/authority/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authority/info.action?id='+data.id
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/authority/delete.action?id="+data.id);
                        layer.close(index);
                        location.reload();
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/authority/input.action?id='+data.id
                    });
                }
            });
