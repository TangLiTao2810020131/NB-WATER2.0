                        ...
                        <div class="layui-card">
                            <div class="layui-card-body">
                                ...
                                <div class="layui-btn-group">
                                    ...
                                    <shiro:hasPermission name="角色管理_批量删除">
                                    	<button class="layui-btn" data-method="deleteUsers"><i class="layui-icon">&#xe640;</i>删除</button>
                                    </shiro:hasPermission>
                                    
                                    <shiro:hasPermission name="角色管理_分配权限">
                                    	<button class="layui-btn" data-method="inRole"><i class="layui-icon">&#xe608;</i>分配权限</button>
                                    </shiro:hasPermission>
                                </div>
                                </div>
...
<script type="text/html" id="handle">
	...
	<shiro:hasPermission name="角色管理_编辑">
    ...
    function tableData() {
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                    ,{field: 'rolename', title: '角色名称', minWidth:120,width:avg+22,align : 'center',unresize:true}
                    ,{title: '操作',width:avg+22,toolbar:'#handle',align : 'center',unresize:true}
                ]]
                ,url: '${ctx}/role/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [15,20,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                ...
                inRole:function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/role/inAuthority.action?id='+data[0].id
                    });
                },
                deleteUsers:function(){
                    ...
                    layer.confirm("删除该角色后,相关用户不可操作，是否确认删除!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/role/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/role/info.action?id='+data.id
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/role/delete.action?id="+data.id);
                        layer.close(index);
                        location.reload();
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/role/input.action?id='+data.id
                    });
                }
            });
