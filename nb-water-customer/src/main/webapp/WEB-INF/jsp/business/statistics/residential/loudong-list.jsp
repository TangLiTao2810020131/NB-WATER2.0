	...
	function tableD() {
		...
		layui.use('table', function() {
			...
			table.render({
				...
				url : '${ctx}/residentialstatistic/ldlistData.action?id=${id}',
				id : 'myTable',
				page : true, //是否显示分页
				limits : [ 15, 20, 50 ],
				limit : 15, //每页默认显示的数量
			...
			table.on('tool(myTable)', function(obj) {
				...
				if (obj.event === 'tubiao') {
					parent.layer.open({
						...
						fixed : false, //不固定
						maxmin : true,
						content : '${ctx}/residentialstatistic/ldtubiao.action?id=' + data.ldid
					});
				}
				if (obj.event === 'liebiao') {
					parent.layer.open({
						...
						fixed : false, //不固定
						maxmin : true,
						content : '${ctx}/residentialstatistic/ldliebiao.action?id=' + data.ldid
					});
				}
			});
