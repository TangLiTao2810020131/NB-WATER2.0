...
<script>
	...
	layui.use('table', function() {
		...
		table.render({
			...
			url : '${ctx}/residentialstatistic/listDataDay.action?id=${id}', //,where: {username: 'admin', id: 123} //如果无需传递额外参数，可不加该参数 //method: 'post' //如果无需自定义HTTP类型，可不加该参数 //,skin: 'line' //表格风格
			id : 'myTable',
			page : true, //是否显示分页
			limits : [ 15, 20, 50 ],
			limit : 8, //每页默认显示的数量
	...
	layui.use('table', function() {
		...
		table.render({
			...
			url : '${ctx}/residentialstatistic/listDataYear.action?id=${id}', //,where: {username: 'admin', id: 123} //如果无需传递额外参数，可不加该参数 //method: 'post' //如果无需自定义HTTP类型，可不加该参数 //,skin: 'line' //表格风格
			id : 'myTable1',
			page : true, //是否显示分页
			limits : [ 15, 20, 50 ],
			limit : 9 //每页默认显示的数量
	...
	function downloadDay(){
		...
		var url = "${ctx}/residentialstatistic/exportDay.action?" + "stime=" + startdateday + "&etime=" + enddateday + "&id=" + id;
	...
	function downloadYear(){
		...
		var id = '${id}';
		//var data = {startdate:startdate,enddate:enddate,useraccount:useraccount,areaid:areaid};
		var url = "${ctx}/residentialstatistic/exportYear.action?" + "year=" + year + "&id=" + id;
		window.location.href = url;
	}
</script>