					...
					<div class="layui-tab" lay-filter="main_tab1">
						...
						<div class="layui-tab-content">
							...
							<div class="layui-tab-item">
								<div class="layui-form layuiadmin-card-header-auto marginBottom">
									<div class="demoTable" style="float: right;" id="demoTableYear">
...
<script>
	...
	layui.use('table', function() {
		...
		table.render({
			...
			url : '${ctx}/ownerstatistic/listDataDay.action?imei=${imei}', //,where: {username: 'admin', id: 123} //如果无需传递额外参数，可不加该参数 //method: 'post' //如果无需自定义HTTP类型，可不加该参数 //,skin: 'line' //表格风格
			id : 'myTable',
			page : true, //是否显示分页
			limits : [ 15, 20, 50 ],
			limit : 8, //每页默认显示的数量
	...
	layui.use('table', function() {
		...
		table.render({
			...
			url : '${ctx}/ownerstatistic/listDataYear.action?imei=${imei}', //,where: {username: 'admin', id: 123} //如果无需传递额外参数，可不加该参数 //method: 'post' //如果无需自定义HTTP类型，可不加该参数 //,skin: 'line' //表格风格
			id : 'myTable1',
			page : true, //是否显示分页
			limits : [ 15, 20, 50 ],
			limit : 9 //每页默认显示的数量
	...
	function downloadDay(){
		...
		var url = "${ctx}/ownerstatistic/exportDay.action?" + "stime=" + startdateday + "&etime=" + enddateday + "&imei=" + imei;
	...
	function downloadYear(){
		...
		var imei = '${imei}';
		//var data = {startdate:startdate,enddate:enddate,useraccount:useraccount,areaid:areaid};
		var url = "${ctx}/ownerstatistic/exportYear.action?" + "year=" + year + "&imei=" + imei;
		window.location.href = url;
	}
</script>
</html>