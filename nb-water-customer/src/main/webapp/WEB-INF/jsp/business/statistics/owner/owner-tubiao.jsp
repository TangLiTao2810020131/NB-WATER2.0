				...
				<div id="nav1">
					...
					<div class="layui-tab" lay-filter="main_tab1">
						<ul id="main_tab1" class="layui-tab-title">
							<li lay-id="0" id = "liDay" class="layui-this">近七日</li>
...
<script>
	...
	function initDay(stime,etime) {
		$.ajax({
			type : "POST", //提交方式 
			url : '${ctx}/ownerstatistic/mainLineDay.action',
			data : {
               imei :  '${imei}',
               stime : stime,
               etime : etime
	...
	function initMonth(year) {
		$.ajax({
			type : "POST", //提交方式 
			url : '${ctx}/ownerstatistic/mainLineMonth.action',
			data : {
               imei :  '${imei}',
               year : year
            }, 
	...
	function ititDayWaterTB(categories, series) {
		...
		var option = {
			title : {
				text : '近'+categories.length+'日户主用水量',
				x : 'center',
				y : 'bottom',
				textAlign : 'center'
			},
