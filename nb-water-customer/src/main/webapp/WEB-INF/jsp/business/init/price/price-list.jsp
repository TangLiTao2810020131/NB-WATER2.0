...
<script>
    ...
    function tableData() {
        ...
        avg = (clientWidth - 138) /3;//分割6等分，表示表格的六列
        ...
        layui.use(['table','layer'], function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                    ,{field: 'type', title: '收费类型', minWidth:120,width:avg,align:'center'}
                    ,{field: 'price', title: '价格', minWidth:120,width:avg,align:'center'}
                    ,{title: '操作', width:avg-20,toolbar:'#handle',align:'center'}
                ]]
                ,url: '${ctx}/price/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [15,20,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                addProvince: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/price/input.action'
                    });
                },
                ...
                deleteProvinces:function(){
                    ...
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/price/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        ...
                        $.post("${ctx}/price/delete.action?id="+data.id);
                        layer.msg('操作成功',{
                            icon:1,
                            time:2000
                        },function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/price/input.action?id='+data.id
                    });
                }
            });
