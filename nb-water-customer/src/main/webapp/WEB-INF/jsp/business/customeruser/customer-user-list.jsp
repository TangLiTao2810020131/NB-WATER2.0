    ...
    function tableData() {
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                    ,{field: 'username', title: '账号', minWidth:120,width:avg
                        ,templet: function(d){
                    ...
                    ,{field: 'realname', title: '姓名', minWidth:120,width:avg,align:'center'}
                    ,{field: 'type', title: '类型', minWidth:120,width:avg
                        ,templet: function(d){
                    ...
                    ,{field: 'tel', title: '电话号码', minWidth:120,width:avg,align:'center'}
                    ,{field: 'isopen', title: '状态', minWidth:120,width:avg
                        ,templet: function(d){
                    ...
                    ,{title: '操作', width:avg+17,toolbar:'#handle',align:'center'}
                ]]
                ,url: '${ctx}/customerUser/listData.action?cid=${cid}'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [10,20,30]
                ,limit:10 //每页默认显示的数量
            ...
            active = {
                addProvince: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/customerUser/input.action?cid=${cid}'
                    });
                },
                ...
                deleteProvinces:function(){
                    ...
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/customerUser/delete.action"+"?mark=customer",//路径
                ...
                restPassword:function(){
                    ...
                    layer.confirm("请问是否重置密码!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/customerUser/restPassword.action",//路径
                            data : {"id":ids},//数据，这里使用的是Json格式进行传输
                            dataType:"json",
                            success : function(result) {//返回数据根据结果进行相应的处理
                                //alert("操作成功");
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/customerUser/info.action?id='+data.id+"&mark=customer"
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/customerUser/delete.action?id="+data.id+"&mark=customer");
                        layer.msg('操作成功',{
                            icon:1,
                            time:2000
                        },function(){
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/customerUser/input.action?id='+data.id+"&cid=${cid}"+"&mark=customer"
                    });
                }
