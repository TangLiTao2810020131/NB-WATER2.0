...
<!--<script type="text/html" id="handle">
  <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">设置地市</a>
...
<script type="text/html" id="handle">
    <a lay-event="detail" title="设置地市" style="cursor:pointer; margin-right:5px;"><i class="layui-icon">&#xe715;</i></a>
...
<script>
    ...
    function tableData() {
        ...
        avg = (clientWidth) / 3;//分割3等分，表示表格的3列
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,cols: [[ //标题栏
                    {type:'checkbox'}
                    ,{field: 'provinceid', title: '地区代码', minWidth:120,width:avg,align : 'center',unresize:true}
                    ,{field: 'province', title: '名称',minWidth:120,width:avg,align : 'center',unresize:true}
                    ,{title: '操作', width:avg-157,toolbar:'#handle',align:'center'}
                ]]
                ,url: '${ctx}/province/listData.action'
                ,id:'myTable'
                ,page: true //是否显示分页
                ,limits: [15,30,50]
                ,limit:15 //每页默认显示的数量
            ...
            active = {
                addProvince: function(){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/province/input.action'
                    });
                },
                ...
                deleteProvinces:function(){
                    ...
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        ...
                        $.ajax({
                            type : "POST", //提交方式
                            url : "${ctx}/province/delete.action",//路径
            ...
            table.on('tool(myTable)', function(obj){
                ...
                if(obj.event === 'detail'){
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/city/list.action?father='+data.provinceid
                    });
                } else if(obj.event === 'del'){
                    layer.confirm("请问是否确定删除，删除后不可恢复!", {
                        btn: ["确定","取消"] //按钮
                    }, function(index){
                        $.post("${ctx}/province/delete.action?id="+data.id);
                        layer.close(index);
                        location.reload();
                    });
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '${ctx}/province/input.action?id='+data.id
                    });
                }
            });
